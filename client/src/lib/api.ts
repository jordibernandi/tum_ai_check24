...
export const getHotelRecommendations = async (prompt: string): Promise<{ status: number, hotels: Hotel[] }> => {
  try {
    const response = await api.post('/recommend_hotels', { "user_prompt": prompt });
